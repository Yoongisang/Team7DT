...
#include "Team7DT/Team7DT.h"
#include "Team7DT/Team7DTPawn.h"
...
void USplineFollowerComponent::BuildPath()
{
	...
	if (!SplinesComp)
	{
		UE_LOG(LogPathFollowingComponent, Warning, TEXT("SplineFollower: No ALandscapeSplineActor found"));
		return;
	}
...
{
	Super::BeginPlay();
	OwnerPawn = Cast<ATeam7DTVehicle>(GetOwner());
	BuildPath();
}
